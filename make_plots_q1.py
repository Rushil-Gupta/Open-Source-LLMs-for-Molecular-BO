...
datasets = ["esol", "freesolv", "nextgen"]
methods = ["coreset", "gp"]
embeds = ["molformer", "llama", "mistral", "qwen"]
...
for j, data in enumerate(datasets):
    for i, meth in enumerate(methods):
        fname = f"diff_embeds_{meth}_{data}"
        fig, ax = plt.subplots(figsize=(10, 6))
        for k, em in enumerate(embeds):
            try:
                with open(f"./results/{data}/{meth}_{em}.txt", "r") as f:
                    file_str = f.readline()
                    rd_wise_frac = [
                        float(mean_str) for mean_str in file_str[1:-2].split()
                    ]
                    rd_wise_frac = np.cumsum(
                        np.round(np.array(rd_wise_frac), 2)
                    ).tolist()
                ax.plot(
                    x_vals,
                    rd_wise_frac,
                    color=color_list[k],
                    label=em_label_dict[em],
                    marker="o",
                    markersize=7,
                    linewidth=2.5,
                )
